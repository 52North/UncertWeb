...
OpenLayers.SOS.ScaleBar = OpenLayers.Class({
	...
	update: function (v) {
		...
		if (v.uom != undefined && v.uom != null) this.uom = v.uom;
	...
	getStyle: function () {
		var self = this;
		function getRule(color, lower, upper ) {
			var filters = [];
			if (upper === 0 || upper) {
		...
		if (!this.style) {
			...
			rules.push(getRule(this.color.less, undefined, this.min));
			for (var i = 0; i < (this.ints-1); i++) {
				var lower = this.min + (i * intervalSize);
				rules.push(getRule(this.getColor(lower), lower, lower + intervalSize));
			}
			rules.push(getRule(this.getColor(this.max), this.max, undefined));
	...
	getSld: function() {
		if (this.sld == null) {
			var entries = [], rules = this.getStyle().rules, last = rules.length-1, sld;
			for (var i = 0; i <= last; i++) {
				var q = (i == last) ? Number.MAX_VALUE : rules[i].filter.filters[0].value;
				entries.push({
					"color": rules[i].symbolizer.color,
					"quantity": q,
					"opacity": 0.8
				});
			...
			this.sld = new OpenLayers.Format.SLD.Custom().write(sld);
		}
		return this.sld;
	}
});
