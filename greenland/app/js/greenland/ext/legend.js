...
Ext.ux.VIS.Legend = Ext.extend(Ext.Panel, {
    ...
    createLegendItemForLayer : function(layer) {
    	if (layer instanceof OpenLayers.Layer.VIS.Raster || layer instanceof OpenLayers.Layer.VIS.Vector
    		...
    		var paramItems = VIS.createParameterControls(options, function() {
    		...
    		tools.push({
    			...
    			handler : function(event, toolEl, panel) {
    				var layer = panel.layer;
    				new Ext.ux.VIS.LayerSettingsWindow({
    					layer : layer,
...
Ext.ux.VIS.Legend.DropZone = Ext.extend(Ext.dd.DropTarget, {
    ...
    notifyDrop : function(dd, e, data) {
    	...
    	var pos = this.lastPos.p, panel = dd.panel, dropEvent = this.createEvent(dd, e, data, pos !== false ? pos
    			: this.legend.items.getCount());
