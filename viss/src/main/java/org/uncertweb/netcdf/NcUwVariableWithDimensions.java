...
//				log.debug("{} == {} = {}", new Object[] { to, t, to.equals(t) });
	...
	public Envelope getEnvelope() {
		if (this.envelope == null) {
			...
			final double xDiff = (xMax - xMin) / (xSize - 1) / 2;
			final double yDiff = (yMax - yMin) / (ySize - 1) / 2;
			...
			this.envelope = new Envelope2D(getCRS(), xMin, yMin, xMax - xMin,
					yMax - yMin);
		}
	...
	private void findDimensions() {
		...
		if (getDimensions().size() != existingDimensions) {
			throw new NcUwException(
					...
					getDimensions(), existingDimensions);
		}
	...
	private void setIndex(NcUwDimension d, int i) {
		if (hasDimension(d)) {
			throw new NcUwException(
	...
	public GridCoordinates getGridCoordinates(final int x, final int y) {
		...
		case UPPER_LEFT:
			break;
		case UPPER_RIGHT:
			gx = getSize(NcUwDimension.X) - x - 1;
			break;
		case LOWER_LEFT:
			gy = getSize(NcUwDimension.Y) - y - 1;
			break;
		case LOWER_RIGHT:
			gx = getSize(NcUwDimension.X) - x - 1;
			gy = getSize(NcUwDimension.Y) - y - 1;
		...
		return new GridCoordinates2D(gx, gy);
	}
...
//		log.debug("Getting value: {}", c);
	...
	public SpatialSamplingFeature getFeature(NcUwCoordinate c) {
		if (c == null || !c.hasDimension(NcUwDimension.X, NcUwDimension.Y)) {
			return new SpatialSamplingFeature(
					NcUwConstants.DEFAULT_SAMPLING_FEATURE,
					NcUwHelper.envelopeToWgs84Polygon(getEnvelope()));
		} else {
			final Point p = NcUwHelper.positionToWgs84Point(getDirectPosition(c));
			return new SpatialSamplingFeature(NcUwConstants.DEFAULT_SAMPLING_FEATURE, p);
		}
	}
