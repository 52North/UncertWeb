...
/**
 * Class representing a Poisson distribution with rate parameter.
 *
 * {@URL https://wiki.aston.ac.uk/foswiki/bin/view/UncertWeb/Poisson}
 *
 * @author Matthew Williams
 * @version 2.0
 */
public class PoissonDistribution implements IContinuousDistribution {
     ...
     * Poisson distributions.
     */
    public PoissonDistribution(List<Double> rate) {
        Validate.notNull(rate);
        Validate.noNullElements(rate);
        Validate.allElementsGreaterThan(rate, 0.0);
        this.rate = new ArrayList<Double>(rate);
    }

    /**
     *
     * @return the rate parameter of each exponential distributions
