...
public class UncertWebBinaryGenerator extends AbstractBinaryGenerator implements IStreamableGenerator{
	...
	public OutputStream generate(IData data) {
		...
		if(!(data instanceof UncertWebDataBinding)){
			throw new RuntimeException("UncertWebBinaryGenerator does not support incoming datatype");
		}
		
		InputStream theStream = ((UncertWebDataBinding)data).getPayload().dataStream;
	...
	public File generateFile(IData data, String mimeType) {
		...
		UncertWebData currentFD = (UncertWebData)data.getPayload();
