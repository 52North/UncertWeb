...
/**
 * Class MerretsAustalAlgo2
 * 
 * Roughly:
 * This class is a WPS process that gets O&M input containing probability
 * distributions. It samples from these distributions, using the UTS. It 
 * returns the samples as O&M, ready to be given to Austal WPS. In future,
 * it should also run the austal WPS.
 * 
 * @author Merret Buurman, August 2011
 *
 */
public class MerretsAustalAlgo2 extends AbstractAlgorithm {
	
	// Path to resources
	private String localPath = "D:\\JavaProjects\\ups";
	private String resPath = localPath + "\\src\\main\\resources\\austalResources";

	// Attributes
	// Store PM10 distributions and emissions:
	private LinkedList<DayDistribution> daydist = new LinkedList<DayDistribution>(); // day sum distributions for each source / each day
	private LinkedList<HourDistribution> hourdist = new LinkedList<HourDistribution>(); // hour fraction distributions for each source / each day
	private LinkedList<DayEmissions> emissions = new LinkedList<DayEmissions>(); // emission values for each source / each day
	private ArrayList<String> emissionsFiles = new ArrayList<String>();
	// Store meteorology
	private LinkedList<MeteoObject> meteoObservations = new LinkedList<MeteoObject>();
	private ArrayList<String> meteoFiles = new ArrayList<String>();
	...
	private int numberOfRealisations = -999; // how often to call the Austal WPS?
	...
	private String inputIDServiceURL = "ServiceURL"; // autal service URL
	private String inputIDIdentifierSimulatedProcess = "IdentifierSimulatedProcess"; // austal process identifier
	private String inputIDUncertainProcessInputs = "UncertainProcessInputs"; // uncertain stuff, treated via UTS
	private String inputIDStaticProcessInputs = "StaticProcessInputs"; // certain stuff, only durchgereicht
...
//		return null;
	...
	public Map<String, IData> run(Map<String, List<IData>> inputMap) {
		...
		System.out.println("Start of process"); // for debugging
		...
		list = inputMap.get("UncertainProcessInputs");
		
		// What if no uncertain process inputs!
		if (list == null){
			emissionsExist = false;
			meteoExist = false;
		
		// What if there is uncertain process inputs:
		} else {
			...
			try {
				obsPropPM10 = new URI("http://www.uncertweb.org/phenomenon/pm10emissions");
				obsPropWindDir = new URI("http://www.uncertweb.org/phenomenon/winddirection");
				obsPropWindSpeed = new URI("http://www.uncertweb.org/phenomenon/windspeed");
			} catch (URISyntaxException e) { // Auto-generated catch block
				e.printStackTrace(); 
			}
			// transform input data to 2 UncertaintyObservationCollections
			// one for emissions, one for meteo (no matter how many O&M files were provided)
			List<UncertaintyObservation> allPM10Observations = null;
			List<UncertaintyObservation> allMeteoObservations = null;
			for (int i = 0; i < list.size(); i++){
				
				// extracting the data
//				String string = (((UncertainInputDataBinding) list.get(i)).getPayload()).getData().xmlText();
				UncertainInputDataBinding uidb = (UncertainInputDataBinding) list.get(i);
				UncertainInputType uit = uidb.getPayload();
				DataType dt = uit.getData();
				String string = dt.xmlText();
				XBObservationParser parser = new XBObservationParser();
				IObservationCollection obsColl = null;
				try {
					obsColl = parser.parse(string);
				} catch (OMParsingException e) { // Auto-generated catch block
				}
				UncertaintyObservationCollection currentUOC = (UncertaintyObservationCollection) obsColl;
				
				// old version
//				IData currentI = list.get(i);
//				OMDataBinding currentOMDB = (OMDataBinding) currentI;
//				OMData currentOMD = (OMData) currentOMDB.getPayload();
//				UncertaintyObservationCollection currentUOC = (UncertaintyObservationCollection) currentOMD.getObservationCollection();
					
				// differentiate the input type:
				boolean emis = currentUOC.getMembers().get(0).getObservedProperty().equals(obsPropPM10);
				boolean winddir = currentUOC.getMembers().get(0).getObservedProperty().equals(obsPropWindDir);
				boolean windspeed = (currentUOC.getMembers().get(0).getObservedProperty().equals(obsPropWindSpeed));
				
				// put all PM10 observations into one list
				if (emis){
					if (allPM10Observations == null){
						allPM10Observations = currentUOC.getMembers();
					} else {
						allPM10Observations.addAll(currentUOC.getMembers());
					}
				// put all wind observations into one list	
				} else if ( winddir || windspeed ){
					if (allMeteoObservations == null){
						allMeteoObservations = currentUOC.getMembers();
					} else {
						allMeteoObservations.addAll(currentUOC.getMembers());
					}
				}
			}
			
			// Booleans so that only the existing inputs are treated...
			if (allPM10Observations == null){
				emissionsExist = false;
			} else {
				emissionsExist = true;
			...
			if (emissionsExist){
				...
				/* for each observation, a new object is created - for the
				 * Daily Emission observations, a DayDistribution object, for
				 * the Hourly Fractions of Daily Emissions, a HourDistributions
				 * object is created. Differentiation by unit of measurement,
				 * once that is implemented.
				 * 
				 */
				for (UncertaintyObservation uncObs : allPM10Observations){
					
					Identifier source = uncObs.getFeatureOfInterest().getIdentifier();
					
					
					/*
					 * So far, unit of measurement in O&M is not supported by out profile 
					 * yet, i.e. the whole O&M won't be read if there is a unit in it. 
					 * Christoph was/is working on this.
					 * So right now, I differentiate by distribution type and then write
					 * the unit of measurement into the observation. This bit below can 
					 * simply be deleted (TODO) without replacement once the units can be 
					 * read from the O&Ms.
					 */
					//    <delete>
					...
					} else if (iUn instanceof MultivariateNormalDistribution){ // Hourly Fractions
					...
					/* In DayDistribution objects, the whole observation is
					 * stored, so that all necessary information can be retrieved
					 * from there later on (when writing O&M again). In 
					 * HourDistribution, only the distribution is stored,
					 * otherwise it would be redundant/overhead.
					 */
					String uom = uncObs.getResult().getUnitOfMeasurement();
					if (uom.equals("g[PM10]/s")){ // Day distribution
						daydist.add(new DayDistribution(source, uncObs.getPhenomenonTime(), uncObs));
					} else if (uom.equals("%")){ // Hour distribution	
						IMultivariateDistribution iMulti = (IMultivariateDistribution) uncObs.getResult().getUncertaintyValue();
						hourdist.add(new HourDistribution(source, uncObs.getPhenomenonTime(), iMulti));
					}
				}
				...
				for (DayDistribution dd: daydist){
					...
					double[] daySamples = callUTSforDAY(distribution); // length of array: [numberOfRealisations]
				...
				for (HourDistribution hd: hourdist){
					...
					double[][] hourSamples = callUTSforHOURS(distribution, 24); // length of array: [numberOfRealisations][24]
				...
				/* Write each O&M observation collection to one XML file.
				 * This is not necessary for running the process, it is only
				 * checking if they work allright.
				 */

				// Make O&M writer and O&M encoder
				IObservationEncoder encoder = new XBObservationEncoder();
				...
				for (int i = 0; i < numberOfRealisations; i++){
					...
					// make file
					//String filepath = "D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\output_om\\Streets";
					...
					String toFile = "";
					try {
						toFile = encoder.encodeObservationCollection(ioc);
					} catch (OMEncodingException e) {
						e.printStackTrace();
					}
					emissionsFiles.add(toFile);
					
					// make writers, write
					try {
						BufferedWriter bw_om = new BufferedWriter(new FileWriter(file));
						bw_om.write(toFile);
						bw_om.flush();
						bw_om.close();
					} catch (IOException e) {
			...
			if (meteoExist){
				...
				for (UncertaintyObservation uncObs : allMeteoObservations){
				...
				// Make OM encoder and O&M parser
				IObservationEncoder encoder = new XBObservationEncoder();
				...
				for (int i = 0; i < numberOfRealisations; i++){
					...
					// make file
					//String filepath = "D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\output_om\\Meteo";
					String filepath = resPath + "\\output_om";	
					filepath = filepath.concat((i+1)+".xml");
					...
					meteoFiles.add(toFile);
					
					// make writers, write
					try {
						BufferedWriter bw_om = new BufferedWriter(new FileWriter(om_file));
						bw_om.write(toFile);
						bw_om.flush();
						bw_om.close();
					} catch (IOException e) {
		...
		/* TODO
		 * Here, treat the austal results and make e.g. distribution
		 * out of them. Then make the OutputMap to return!
		 */
	 ...
	 * 
	 * 
	 * This methods calls the UTS (LogNormalDistribution)
	 * and returns samples.
	 * 
	 * Functions well (17 August 2011).
	 * XML handling could be done more elegantly.
	 * 
	 * @param  shape    The shape parameter of the log normal distribution, as a string.
	 * @param  lgoScale The logScale parameter of the log normal distribution, as a string.
	 */
	private double[] callUTSforDAY(IUncertainty distribution){
		...
		if (distribution instanceof LogNormalDistribution){
			...
			try {
				//execDocUTSday = ExecuteDocument.Factory.parse(new File("D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\request_templates\\LogNormalRequest.xml"));
				execDocUTSday = ExecuteDocument.Factory.parse(new File(resPath + "\\request_templates\\LogNormalRequest.xml"));
			...
			InputType[] types = execDocUTSday.getExecute().getDataInputs().getInputArray();
			for (InputType inputType : types) {
				String id = inputType.getIdentifier().getStringValue();
				if(id.equals("distribution")){
					
					// Set new logscale
					Node wpsComplexData = inputType.getData().getComplexData().getDomNode(); // Element wps:ComplexData
					Node unLogNormalDistribution = wpsComplexData.getChildNodes().item(1); // Element un:LogNormalDistribution
					Node unlogScale = unLogNormalDistribution.getChildNodes().item(1); // Element un:LogScale
					Node logScaleValueNode = unlogScale.getChildNodes().item(0); // Contents of it
					logScaleValueNode.setNodeValue(logScale);
						
					// Set new shape
					Node unShape = unLogNormalDistribution.getChildNodes().item(3); // Element un:LogScale
					System.out.println(unShape.getNodeName());
					Node shapeValueNode = unShape.getChildNodes().item(0); // Contents of it
					shapeValueNode.setNodeValue(shape);
						
				// set new number of realisations
				} else if(id.equals("numbReal")){
					Node wpsLiteralData = inputType.getData().getLiteralData().getDomNode();
					Node wpsLiteralDataValueNode = wpsLiteralData.getChildNodes().item(0);
					String newNumbReal = ((Integer) numberOfRealisations).toString();
		...
		try {
			responseUTSday = (ExecuteResponseDocument) session.execute("http://localhost:8080/uts/WebProcessingService", execDocUTSday);
		} catch (WPSClientException e) {
				e.printStackTrace();
		}
		...
		OutputDataType oType = responseUTSday.getExecuteResponse().getProcessOutputs().getOutputArray(0); // all output elements
		Node wpsComplexData = oType.getData().getComplexData().getDomNode(); // the complex data node
		Node unRealisation = wpsComplexData.getChildNodes().item(0); // the realisation node
		Node unValues = unRealisation.getChildNodes().item(3); // the values node
		Node realisationsValueNode = unValues.getChildNodes().item(0); // the node with the contents of the value node
		String realisationsValue = realisationsValueNode.getNodeValue(); // the value of the contents
	 ...
	 * 
	 * 
	 * This methods calls the UTS (MultivariateNormalDistribution)
	 * and returns samples. 
	 * 
	 * Functions well (17 August 2011).
	 * XML handling could be done more elegantly.
	 * 
	 * @param  dimensions The dimensions of the multivariate distribution. Should be 24.
	 * @param  meanVector The mean vector of the multivariate gaussian distribution, as a 
	 *         atring, readily formatted to be given to the UTS as UncertML input (all
	 *         values separated by space)
	 * @param  covMatrix The covariance matrix of the the multivariate gaussian distribution,
	 * 		   as a atring, readily formatted to be given to the UTS as UncertML input (all
	 *         values separated by space)
	 * 
	 ...
	 *  	   distribution).
	 */
	private double[][] callUTSforHOURS(IMultivariateDistribution distribution, int dimensions){
		...
		if (distribution instanceof MultivariateNormalDistribution){
			...
			meanVector = meanVector.substring(1, meanVector.length()-1); // to remove "[", "]"
			meanVector = meanVector.replace(",", ""); // to remove ","
			// cov matrix as String
			String covMatrix = distrib.getCovarianceMatrix().getValues().toString(); // This should look like [x, y, z]
			covMatrix = covMatrix.substring(1, covMatrix.length()-1); // Now like this: x, y, z
			covMatrix = covMatrix.replace(",", ""); // to remove ","
				
			// Make execute request
			try {
				//execDocUTShours = ExecuteDocument.Factory.parse(new File("D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\request_templates\\MultivariateRequest.xml"));
				execDocUTShours = ExecuteDocument.Factory.parse(new File(resPath + "\\request_templates\\MultivariateRequest.xml"));
			} catch (XmlException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
			}
			InputType[] types = execDocUTShours.getExecute().getDataInputs().getInputArray();
			...
			for (InputType inputType : types) {
				...
				if(id.equals("numbReal")){
					...
					// Set new mean vector
					Node wpsComplexData = inputType.getData().getComplexData().getDomNode(); // Element wps:ComplexData
					Node unMultivariateNormalDistribution = wpsComplexData.getChildNodes().item(1); // Element un:MultivariateNormalDistributio
					Node unMean = unMultivariateNormalDistribution.getChildNodes().item(1); // Element un:Mean
					Node meanValueNode = unMean.getChildNodes().item(0); // its contents
					meanValueNode.setNodeValue(meanVector);
					
					// Set new covariance matrix
					Node unCovMat = unMultivariateNormalDistribution.getChildNodes().item(3); // Element un:CovarianceMatrix
					Node unValues = unCovMat.getChildNodes().item(1); // Element values
		...
		try {
			responseUTShours = (ExecuteResponseDocument) session.execute("http://localhost:8080/uts/WebProcessingService", execDocUTShours);
		...
		// Get realisations out of response xml
		// TODO This could be done more elegantly, maybe?
		OutputDataType oType = responseUTShours.getExecuteResponse().getProcessOutputs().getOutputArray(0); // all output elements
		Node wpsComplexData = oType.getData().getComplexData().getDomNode(); // the complex data node
		Node unRealisation = wpsComplexData.getChildNodes().item(0); // the realisation node
		Node unValues = unRealisation.getChildNodes().item(3); // the values node
		Node realisationsValueNode = unValues.getChildNodes().item(0); // the node with the contents of the value node
	 ...
	 * 
	 * 
	 * This methods calls the UTS (NormalDistribution or potentially others)
	 * and returns samples.
	 * 
	 * XML handling could be done more elegantly.
	 * 
	 * @param  distribution  This is any distribution. So far, only 
	 * 						 NormalDistribution is implemented.
	 */
	private double[] callUTSforMeteo(IUncertainty distribution) {
		...
		if (distribution instanceof NormalDistribution){
			...
			// extract parameters
			String meanString = ((NormalDistribution) distribution).getMean().get(0).toString();
			String varString = ((NormalDistribution) distribution).getVariance().get(0).toString();
			
			// Make execute request
			try {
				//execDocUTSmeteo = ExecuteDocument.Factory.parse(new File("D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\request_templates\\NormalRequest.xml"));
				execDocUTSmeteo = ExecuteDocument.Factory.parse(new File(resPath + "\\request_templates\\NormalRequest.xml"));
			} catch (XmlException e) {
				e.printStackTrace();
			} catch (IOException e) {
				e.printStackTrace();
			...
			InputType[] types = execDocUTSmeteo.getExecute().getDataInputs().getInputArray();
			for (InputType inputType : types){
				String id = inputType.getIdentifier().getStringValue();
				if(id.equals("distribution")){
					...
					// Set new mean
					Node wpsComplexData = inputType.getData().getComplexData().getDomNode(); // Element wps:ComplexData
					Node unNormalDistribution = wpsComplexData.getChildNodes().item(1); // Element un:LogNormalDistribution
					Node unMean = unNormalDistribution.getChildNodes().item(1); // Element un:LogScale
					Node unMeanValueNode = unMean.getChildNodes().item(0); // Contents of it
					unMeanValueNode.setNodeValue(meanString);
						
					// Set new variance	
					Node unVariance = unNormalDistribution.getChildNodes().item(3); // Element un:LogScale
					Node varianceValueNode = unVariance.getChildNodes().item(0); // Contents of it
		...
		try {
			responseUTSmeteo = (ExecuteResponseDocument) session.execute("http://localhost:8080/uts/WebProcessingService", execDocUTSmeteo);
		} catch (WPSClientException e) {// Auto-generated catch block
			e.printStackTrace();
		}
		...
		OutputDataType oType = responseUTSmeteo.getExecuteResponse().getProcessOutputs().getOutputArray(0); // all output elements
		Node wpsComplexData = oType.getData().getComplexData().getDomNode(); // the complex data node
		Node unRealisation = wpsComplexData.getChildNodes().item(0); // the realisation node
		Node unValues = unRealisation.getChildNodes().item(3); // the values node
		Node realisationsValueNode = unValues.getChildNodes().item(0); // the node with the contents of the value node
	...
	private void computeEmissions(){
		...
		// Combine daily emission (sum) and hour fractions to get hourly emissions
		...
		for (DayDistribution dd: daydist){
			...
			for (HourDistribution hd: hourdist){
				...
				if (  (ddSource.equals(hdSource)) &&
					...
					for (int i = 0; i < numberOfRealisations; i++){
						for (int j = 0; j < 24; j++){
							hourlyEmissions[i][j] = dayEmissions[i] * hourFractions[i][j];
						}
					}
					DayEmissions dayEmis = new DayEmissions(hourlyEmissions, dd.getTime(), dd.getSourceID(), dd.getObservation());
	 ...
	 * The measurements are stored in a MeasurementCollection, which is returned.
	 * 
	 * @param austalrun The number of the austal run. Austal is run n times, and
	 * 					for each time, a different set of emissions is used. So 
	 * 					for each austal run, a different MeasurementCollection
	 * 					is made out of different emissions.
	 */
...
//			String unitOfMeasurement = dayEmis.getUnitOfMeasurement(); // TODO Once unit works, enable this!
			...
			for (int hour = 0; hour < 24; hour++){
				...
				// make result
				MeasureResult result = new MeasureResult(currentEmissions[hour], unitOfMeasurement);
...
//			String unitOfMeasurement = meteo.getUnitOfMeasurement();
			String unitOfMeasurement = "platzhalter unit of measurement"; // TODO Once unit works, enable this!
			MeasureResult result = new MeasureResult(currentValue, unitOfMeasurement);

			// make measurement
			UncertaintyObservation uo = meteo.getUncertaintyObservation();
			Measurement me = new Measurement(identifier, uo.getBoundedBy(), 
					uo.getPhenomenonTime(), uo.getResultTime(), uo.getValidTime(), 
					uo.getProcedure(), uo.getObservedProperty(), uo.getFeatureOfInterest(),
	 ...
	 * It uses locally stored request files, so the paths have to be adapted!
	 */
	private void makeRequestAndRunAustal(int runNumber) {
		...
		try {
			//execDoc = ExecuteDocument.Factory.parse(new File("D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\request_templates\\AustalRequest.xml"));
			execDoc = ExecuteDocument.Factory.parse(new File(resPath + "\\request_templates\\AustalRequest.xml"));
		...
		if(emissionsExist){ // if emissions exist
			// there can be only one per austal run, as all emissions are collected in one MeasurementCollection
			// emissions input node ("insert") is made up of start, middle and end:
		...
		if(meteoExist){ // if meteorology exist
			// there can be only one per austal run, as all meteorology observations are collected in one MeasurementCollection
		...
		if(staticInputsExist){ // if static inputs exist
			// there can be several per austal run: receptor points, start-time, end-time, more receptor points...
			int num = staticInputsList.size();
			for (int i = 0; i < num; i++){
				// get static input
				StaticInputDataBinding a = (StaticInputDataBinding) staticInputsList.get(i);
...
//			contentNode.setNodeValue(node);
		...
		// insert this long string to the request
			contentNode.setNodeValue(allInput); //TODO here it puts some non-xml tags around my string...
		
		// write the request to a file (one file per run)
		/* Not needed for running the whole process in the end, but used
		 * to check the correctness of this request until the austal wps
		 * can be used!
		 */
		//String filepath2 = "D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\output_austalrequests\\AustalRequest";
		String filepath2 = resPath + "\\output_austalrequests\\AustalRequest";
		filepath2 = filepath2.concat(runNumber + ".txt");
		File austalfile2 = new File(filepath2);
		try {
			BufferedWriter bw_austaloutput = new BufferedWriter(new FileWriter(austalfile2));
...
//		try {
//			response1 = (ExecuteResponseDocument) session.execute(serviceURL, execDoc);
//		} catch (WPSClientException e) {
//			e.printStackTrace();
//		}
//		
...
//		 * 
//		 */
//		OutputDataType oType = response1.getExecuteResponse().getProcessOutputs().getOutputArray(0); // all output elements
//		Node wpsComplexData = oType.getData().getComplexData().getDomNode(); // the complex data node
//		Node unRealisation = wpsComplexData.getChildNodes().item(0); // the realisation node
//		Node unValues = unRealisation.getChildNodes().item(3); // the values node
//		Node realisationsValueNode = unValues.getChildNodes().item(0); // the node with the contents of the value node
...
//		 * 
//		 */
//		String filepath = "D:\\Eclipse_Workspace\\ups_wrapper\\src\\main\\resources\\AustalOutput";
//		filepath = filepath.concat((runNumber+1) + ".txt");
//		File austalfile = new File(filepath);
//		try {
//			BufferedWriter bw_austaloutput = new BufferedWriter(new FileWriter(austalfile));
