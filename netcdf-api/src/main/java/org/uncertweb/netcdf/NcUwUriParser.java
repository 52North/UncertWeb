...
//		}
	...
	public Percentile parsePercentile() {
		return new Percentile(getDouble(Fragments.LEVEL), getDouble(Fragments.VALUE));
	}

	public Quantile parseQuantile() {
		return new Quantile(getDouble(Fragments.LEVEL), getDouble(Fragments.VALUE));
	}

	public Quartile parseQuartile() {
		return new Quartile(getDouble(Fragments.LEVEL), getDouble(Fragments.VALUE));
	}
	...
	public StatisticCollection parseStatisticCollection() {
		...
		for (URI uri : getValues().keySet()) {
			...
			if (type != null && type != NcUwUncertaintyType.STATISTIC_COLLECTION
					&& type.isStatistic()) {
	...
	protected  final Number getNumber() {
		return (Number) getValues(getType().getUri()).get(0);
	}
